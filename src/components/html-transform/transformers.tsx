...
//Elementos Div, distintas clases
...
export function transformRow (el: Element, options: HTMLReactParserOptions) {
  ...
  const allColsAreHalf = validChildren.every((child) =>
    child.attribs?.class?.includes('col-md-6')
  )

  const rowClasses = allColsAreHalf
    ? 'flex flex-row gap-4 border-primary items-center justify-center'
...
export function transformCol (el: Element, options: HTMLReactParserOptions) {
  return (
    <div className="border-primary my-5 flex items-center justify-between rounded-lg border px-5 py-3">
      {domToReact(el.children as DOMNode[], options)}
    </div>
  )
}
...
export function transformCard (el: Element, options: HTMLReactParserOptions) {
  return (
    <CardShine className='relative mx-auto my-5 flex w-full max-w-[350px] overflow-hidden transition duration-500 hover:scale-105 hover:bg-[var(--color-primary-dark)]'>
      {domToReact(el.children as DOMNode[], options)}
    </CardShine>
  )
}
...
export function transformLi (el: Element, options: HTMLReactParserOptions) {
  return (
    <li className="text-p-custom relative mt-4 pl-6 leading-relaxed text-zinc-200">
...
export function transformCode (el: Element, options: HTMLReactParserOptions) {
  return (
    <code className="mx-2 flex items-end justify-end text-[var(--color-primary)]">
      {domToReact(el.children as DOMNode[], options)}
    </code>
  )
}
